...
-- Concrete actions taken by AI agents (the receipts behind every agent invocation)
...
CREATE INDEX ix_agent_actions_customer ON agent_actions(customer_id, created_at DESC);
CREATE INDEX ix_agent_actions_trace ON agent_actions(trace_id);
CREATE INDEX ix_agent_actions_type ON agent_actions(action_type, created_at DESC);
CREATE INDEX ix_agent_actions_agent ON agent_actions(agent_type, created_at DESC);

-- Real escalation queue — human specialists pick up from here
...
CREATE INDEX ix_escalations_status ON human_escalations(status, urgency, created_at);
CREATE INDEX ix_escalations_customer ON human_escalations(customer_id, created_at DESC);
CREATE INDEX ix_escalations_specialist ON human_escalations(specialist_type, status);

-- Real payment links (issued by agents, tracked through customer click + payment)
...
CREATE INDEX ix_payment_links_customer ON payment_links(customer_id, issued_at DESC);
CREATE INDEX ix_payment_links_status ON payment_links(status, expires_at);

-- Service heartbeats (every service emits one every ~5s; used by Platform Operations page)
